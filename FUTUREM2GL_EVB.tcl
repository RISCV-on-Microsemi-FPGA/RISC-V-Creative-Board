...
if {"$target" == "AHB"} then {
	if {[file exists $project_dir_ahb] == 1} then {
		...
		source ./import/components/AHB/import_component_and_constraints_futurem2gl_ahb.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
	...
	if {[file exists $project_dir_axi] == 1} then {
		...
		source ./import/components/AXI/import_component_and_constraints_futurem2gl_axi.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
