...
proc create_new_project_label_ahb { }\
{
	...
	puts "-------------MiV_RV32IMA_L1_AHB_BaseDesign_FUTUREM2GL_EVB----------------"
    puts "-------------------------------------------------------------------------"
	puts "-------------------------------------------------------------------------"
}
...
proc create_new_project_label_axi { }\
{
    ...
    puts "-------------------------------------------------------------------------"
    puts "-----------------------CREATING NEW PROJECT------------------------------"
	puts "-------------MiV_RV32IMA_L1_AXI_BaseDesign_FUTUREM2GL_EVB----------------"
    puts "-------------------------------------------------------------------------"
	puts "-------------------------------------------------------------------------"
}
...
if {"$target" == "AHB"} then {
	if {[file exists $project_dir_ahb] == 1} then {
		...
		source ./import/components/BaseDesign/AHB/import_component_and_constraints_futurem2gl_ahb.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
	...
	if {[file exists $project_dir_axi] == 1} then {
		...
		source ./import/components/BaseDesign/AXI/import_component_and_constraints_futurem2gl_axi.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
...
# ##
	...
	if {"$target" == "AHB"} then {
		export_prog_job \
			-job_file_name {BaseDesign_FUTUREM2GL-Basic-AHB-ProgramFile} \
		...
		export_prog_job \
			-job_file_name {BaseDesign_FUTUREM2GL-Basic-AXI-ProgramFile} \
			-export_dir {./../FlashPro_Express_Projects/Programming_Files} \
			-bitstream_file_type {TRUSTED_FACILITY} \
			-bitstream_file_components {FABRIC ENVM} \
			-bitstream_file_components {}
